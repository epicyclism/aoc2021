...
template<typename S> bool is_finished(S const& s)
{
    auto& b{ s.first };
    if constexpr (std::is_same_v<S, state_t>)
...
state2_t mk_state2(state_t const& s)
{
    state2_t s2;
    s2.second = 0;
    auto& b2 {s2.first};
...
void generate2(state2_t& s, std::vector<state2_t>& vr)
{
    auto& b{ s.first };
    auto record_move_to_top = [&](int f, int t, char c)
    {
...
int pt2(auto in)
{
    auto s2 { mk_state2({in, 0})};
    std::cout << s2.first << "\n";
    std::map<burrow2_t, std::map<burrow2_t, int>> mp;
    vo.push_back(s2);
    while( !vo.empty())
    {
        for (auto& st : vo)
